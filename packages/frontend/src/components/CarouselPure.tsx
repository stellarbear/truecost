...
export const CarouselPure: React.FC<IProps> = (props) => {
    const {
        children,
        infinite = true,
    } = props;
    ...
    const dots = () => children.length > 1 && (
        <Row style={{position: "absolute", bottom: 8, left: 0, right: 0, }}>
            {
    ...
    const view = () => (
        children[current] && React.isValidElement(children[current]) && React.cloneElement(children[current] as any, {
